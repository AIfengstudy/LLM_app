...
class GzzcaMainwindow(QWidget):
    def __init__(self):
        ...
        self.top_label_widget.setPixmap(QPixmap("external_files/images/topbg.png").scaled(self.width + 10, 100))
    ...
    def _setup_ui(self):
        ...
        self.top_logo_widget.setPixmap(QPixmap("external_files/images/logo_title.png").scaled(500, 70))              # 背景设置
        self.top_logo_widget.setScaledContents(True)                                                    # 自适应设置

        # 设置软件窗口图标
        self.icon = QIcon()
        self.icon.addPixmap(QPixmap("external_files/images/logos.png"), QIcon.Normal, QIcon.Off)
    ...
    def listwidgetFuns(self, item):
        ...
        if item == '软件首页':
            ...
            right_index_label.setPixmap(
                QPixmap(
                    "external_files/images/center_index.png").scaledToWidth(self.width - 100).scaledToHeight(self.height - 130))
            ...
            print('环境部署')
            from modules.EnvBuild.main import MainWindow
            self.firstwindow = MainWindow()
            self.right_widget.addWidget(self.firstwindow)

        elif item == '制数据集':
            print('制数据集')
            from modules.DataMake.main import MainWindow  # 需要调整下 rc文件的导入方式
            self.secondwindow = MainWindow()
            self.right_widget.addWidget(self.secondwindow)

        elif item == '模型微调':
            print('模型微调')
            from modules.FineTuning.main import MainWindow
            ...
            print('模型实训')
            from modules.Traing.main import MainWindow
